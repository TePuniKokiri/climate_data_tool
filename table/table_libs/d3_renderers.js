  ...
  callWithJQuery(function($, d3) {
    return $.pivotUtilities.d3_renderers = {
      Treemap: function(pivotData, opts) {
		...
		var pivotrowKeys = Object.keys(pivotData.tree);
		...
		color = function(key) {
		  var keyIndex = pivotrowKeys.findIndex(x => x === key);
		  return Array(pivotData.rowKeys.length).fill().map((element, index) => d3.scale.linear().domain([0, 0.25, 0.5, 0.75, 1]).range(["#007c9d", "#03b1df", "#bbbbbb", "#ffb975", "#fd8542"])(index / (pivotData.rowKeys.length - 1)))[keyIndex];
		}
        width = opts.d3.width();
